...
<title>VMIS | Account Types</title>
...
<style>
  ...
  .stat-card-modern.at-total .stat-icon-wrapper { background: rgba(59, 130, 246, 0.12); color: #3b82f6; }
  .stat-card-modern.at-builtin .stat-icon-wrapper { background: rgba(139, 92, 246, 0.12); color: #8b5cf6; }
  .stat-card-modern.at-custom .stat-icon-wrapper { background: rgba(245, 158, 11, 0.14); color: #d97706; }
  .stat-card-modern.at-users .stat-icon-wrapper { background: rgba(34, 197, 94, 0.12); color: #16a34a; }
  ...
  @media (max-width: 767.98px) {
    ...
    #accountTypesTable td:nth-child(2)::before { content: "Level"; }
    #accountTypesTable td:nth-child(3)::before { content: "In Use"; }
  }
  ...
  .form-control-modern:disabled { background: #eef1f6; color: #94a3b8; }
</style>
@endsection

@section('nav-title', 'VMIS | Settings > Account Types')

@section('nav-actions')
<button type="button" class="btn-add-entity" data-toggle="modal" data-target="#createAccountTypeModal">
    <i class="fas fa-plus mr-1"></i> <span class="btn-label">Add Account Type</span>
</button>
@endsection
...
<section class="content pt-3">
    <div class="container-fluid">
        ...
        <div class="alert alert-light border small text-muted mb-3" style="border-radius: 10px;">
            ...
            The six built-in roles (Super Administrator, Administrator, Unit Administrator, Station Administrator,
            Viewer, Driver) are locked against renaming or deletion because they're wired directly into every
            page's access control. A custom type you add here can be assigned to a user, but has no specific
            page access anywhere in the system until a developer adds code for it.
        </div>

        <div class="fleet-stats-grid">
            <div class="stat-card-modern at-total">
                <div class="stat-icon-wrapper"><i class="fas fa-user-tag"></i></div>
                <div>
                    <div class="stat-num-value">{{ $stats['total_types'] }}</div>
                    <div class="stat-label-title">Total Account Types</div>
                </div>
            </div>
            <div class="stat-card-modern at-builtin">
                <div class="stat-icon-wrapper"><i class="fas fa-lock"></i></div>
                <div>
                    <div class="stat-num-value">{{ $stats['built_in'] }}</div>
                    <div class="stat-label-title">Built-in Roles</div>
                </div>
            </div>
            <div class="stat-card-modern at-custom">
                <div class="stat-icon-wrapper"><i class="fas fa-tags"></i></div>
                <div>
                    <div class="stat-num-value">{{ $stats['custom_types'] }}</div>
                    <div class="stat-label-title">Custom Types</div>
                </div>
            </div>
            <div class="stat-card-modern at-users">
                <div class="stat-icon-wrapper"><i class="fas fa-users"></i></div>
                <div>
                    <div class="stat-num-value">{{ $stats['total_users'] }}</div>
                    <div class="stat-label-title">Total User Accounts</div>
                </div>
            </div>
        </div>
        ...
        <div class="fleet-card-container">
            <div class="toolbar-header">
                <div class="toolbar-title">
                    <h5><i class="fas fa-user-tag text-primary"></i> Account Types Registry</h5>
                    <p>Every account type a user can be assigned, and how many accounts currently use it</p>
                </div>
                <div class="filter-bar">
                    <div class="search-input-shell">
                        <i class="fas fa-search"></i>
                        <input type="text" id="accountTypeSearchBox" class="form-control" placeholder="Search types...">
                    </div>
                    <button type="button" id="resetAccountTypeFiltersBtn" class="btn btn-light border text-secondary"><i class="fas fa-undo"></i></button>
                </div>
            </div>

            <div class="table-responsive">
                <table class="table fleet-table" id="accountTypesTable">
                    <thead>
                        <tr>
                            <th>Account Type</th>
                            <th>Sort Level</th>
...
<div class="modal fade" id="createAccountTypeModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-content-premium">
            <div class="modal-header-slate d-flex justify-content-between align-items-center">
                <h5><i class="fas fa-user-tag mr-2 text-primary"></i> Add Account Type</h5>
                <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
            </div>
            <form action="{{ route('account-types.store') }}" method="POST" id="createAccountTypeForm">
                @csrf
                <div class="modal-body p-4">
                    <div class="form-group mb-3">
                        <label class="font-weight-bold font-size-12 text-secondary text-uppercase mb-2">Type Name <span class="text-danger">*</span></label>
                        <input type="text" name="type" class="form-control form-control-modern" required maxlength="100" placeholder="e.g. FLEET COORDINATOR" style="text-transform:uppercase;">
                    </div>
                    <div class="form-group mb-0">
                        <label class="font-weight-bold font-size-12 text-secondary text-uppercase mb-2">Sort Level <span class="text-danger">*</span></label>
                        <input type="number" name="level" class="form-control form-control-modern" required min="1" max="999" placeholder="e.g. 7">
                        <small class="text-muted">Controls display order only (lower shows first) — has no effect on access.</small>
                    </div>
                </div>
                <div class="modal-footer bg-light border-0 px-4 py-3">
                    ...
                    <button type="submit" class="btn btn-primary font-weight-bold shadow-sm px-4 py-2" style="border-radius: 9px; background: linear-gradient(135deg, #3b82f6, #1d4ed8); border: none;"><i class="fas fa-check mr-1"></i> Save Type</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="editAccountTypeModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content modal-content-premium">
            <div class="modal-header-slate d-flex justify-content-between align-items-center">
                <h5><i class="fas fa-edit mr-2 text-primary"></i> Edit Account Type</h5>
                <button type="button" class="close text-white" data-dismiss="modal">&times;</button>
            </div>
            <form id="editAccountTypeForm" method="POST">
                @csrf @method('PUT')
                <div class="modal-body p-4">
                    <div id="editBuiltInNotice" class="alert alert-warning border-0 small" style="display:none; background:#fffbeb; color:#92400e;">
                        <i class="fas fa-lock mr-1"></i> This is a built-in system role — only its sort level can be changed.
                    </div>
                    <div class="form-group mb-3">
                        <label class="font-weight-bold font-size-12 text-secondary text-uppercase mb-2">Type Name <span class="text-danger">*</span></label>
                        <input type="text" id="edit_type" name="type" class="form-control form-control-modern" required maxlength="100" style="text-transform:uppercase;">
                    </div>
                    <div class="form-group mb-0">
                        <label class="font-weight-bold font-size-12 text-secondary text-uppercase mb-2">Sort Level <span class="text-danger">*</span></label>
                        <input type="number" id="edit_level" name="level" class="form-control form-control-modern" required min="1" max="999">
                    </div>
                </div>
                <div class="modal-footer bg-light border-0 px-4 py-3">
                    <button type="button" class="btn btn-light font-weight-bold px-4 py-2" data-dismiss="modal" style="border-radius: 9px;">Cancel</button>
...
$(document.body).ready(function() {
    ...
    const accountTypesTable = $('#accountTypesTable').DataTable({
        processing: true,
        serverSide: true,
        autoWidth: false,
        ajax: { url: "{{ route('account-types.index') }}" },
        columns: [
            { data: 'name_html', name: 'type' },
            { data: 'level_html', orderable: false, searchable: false },
            { data: 'usage_html', orderable: false, searchable: false },
        ...
        pageLength: 10,
        language: {
            processing: '<div class="spinner-border spinner-border-sm text-primary" role="status"></div> Loading account types...'
        }
    });

    let searchTimeout;
    $('#accountTypeSearchBox').on('keyup input', function() {
        clearTimeout(searchTimeout);
        let val = $(this).val();
        searchTimeout = setTimeout(function() { accountTypesTable.search(val).draw(); }, 300);
    });

    $('#resetAccountTypeFiltersBtn').on('click', function() {
        $('#accountTypeSearchBox').val('');
        accountTypesTable.search('').draw();
    });

    $('#accountTypesTable').on('click', '.edit-account-type-btn', function() {
        let id = $(this).data('id');
        let isBuiltIn = $(this).data('builtin') == 1;

        $('#edit_type').val($(this).data('type'));
        $('#edit_level').val($(this).data('level'));
        $('#edit_type').prop('readonly', isBuiltIn);
        $('#editBuiltInNotice').toggle(isBuiltIn);

        let updateUrl = "{{ route('account-types.update', ':id') }}".replace(':id', id);
        $('#editAccountTypeForm').attr('action', updateUrl);
        $('#editAccountTypeModal').modal('show');
    });

    $('#accountTypesTable').on('click', '.delete-account-type-btn', function() {
        let id = $(this).data('id');
        ...
        if (count > 0) {
            Swal.fire({
                ...
                text: 'This type is still assigned to ' + count + ' user account(s). Reassign them first.',
                icon: 'error',
        ...
        Swal.fire({
            title: 'Delete ' + type + '?',
            text: "This action is permanent and will remove this account type from the system.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                let deleteUrl = "{{ route('account-types.destroy', ':id') }}".replace(':id', id);
    ...
    $('#createAccountTypeForm').submit(function(e) {
        e.preventDefault();
        let form = this;
        Swal.fire({
            title: 'Save Account Type?',
            text: "Are you sure you want to add this new account type?",
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3b82f6',
            cancelButtonColor: '#64748b',
